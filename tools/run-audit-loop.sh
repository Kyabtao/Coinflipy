...
# Runs the audit harness N times (default 20). Any failing pass aborts with
# exit 1 so a fix + re-run is required, exactly matching the clean-pass contract.
set -u
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
PASSES="${1:-20}"
LOG="$ROOT/tossmatch/docs/audit-loop-v12.0.log"
rm -f "$LOG"
