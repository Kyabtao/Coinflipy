...
import {P2P_EXTRA} from "../player/catalog100.js";
import {EXT_ARCADE2} from "../player/arcade100.js";
...
const FEATURE_DIRECTORY=[
 ...
 {cat:'Core Games',code:'CORE-2',name:'P2P Games',desc:'One hundred proof-driven games with per-game queues, manual take, carries and history.',status:'Implemented',tab:'games'},
 ...
 {cat:'Progression',code:'P7',name:'Daily Missions',desc:'Six rotating daily goals (5 games, 3 wins, catalog, arcade, streak, wager) paying escalating BONUS with a daily reset.',status:'Implemented',tab:'progressionplus',feature:'missions'},
 {cat:'Progression',code:'P8',name:'Badge Case',desc:'Twelve automatic career badges covering games, wins, streaks, jackpots, VIP, arcade and cosmetics.',status:'Implemented',tab:'progressionplus',feature:'badges'},
 ...
 {cat:'Economy+',code:'E11',name:'Bond Market',desc:'30-day income bonds bought with MAIN, 2% annual interest paid to BONUS and principal returned at maturity.',status:'Implemented',tab:'economyplus',feature:'bonds'},
 {cat:'Economy+',code:'E12',name:'Coin Mine',desc:'Dig with daily-refilling energy for 15–40 shard packs at a 60% success rate.',status:'Implemented',tab:'economyplus',feature:'mine'},
 {cat:'Economy+',code:'E13',name:'Coin Exchange',desc:'Convert BONUS into MAIN at a 10% house fee with a 500 BONUS daily cap.',status:'Implemented',tab:'economyplus',feature:'exchange'},
 ...
 {cat:'Player & Retention',code:'RET-3',name:'VIP+ Rewards',desc:'Daily VIP bonus, monthly wagering quest and a per-tier rewards road paid in MAIN + BONUS.',status:'Implemented',tab:'season',admin:'plus'},
 {cat:'Player & Retention',code:'RET-4',name:'Season+',desc:'Season points from games, wins, cups and tournaments; factions, four missions and a tracked prize ladder.',status:'Implemented',tab:'season',admin:'plus'},
 ...
 /* Catalog games CAT37–CAT100 (generated from the player catalog module). */
 ...P2P_EXTRA.map(g=>({cat:'Core Games',code:g.code,name:g.name,desc:g.desc,status:'Implemented',tab:'games',feature:g.id})),
 /* Arcade Zone modes G30–G104 (generated from the Arcade expansion module). */
 ...Object.values(EXT_ARCADE2).map((g,i)=>({cat:'Arcade Zone',code:'G'+(26+i),name:g.title,desc:g.desc,status:'Implemented',tab:'newgames',feature:g.id}))
];
const PAGE_SIZE=20;
...
function defaultState(){return {v:11.0,frozen:{you:false,reason:"",at:0,by:""},config:{stakeMin:10,stakeMax:1000,payoutCap:0,animMs:2300,edgePct:2,feePct:5,cupRakePct:5,trnyRakePct:10,jpFundPct:10,jpFloor:1,jpArm:50,jpPayPct:50,nonMainCapPct:20,transferFee:2,transferMin:10,transferCap:500,
    ...
    vipPlus:{dailyBase:25,dailyPerTier:10,questWager:2000,questRewardPerTier:50},
    seasonPlus:{pointsPerGame:2,pointsPerWin:3,pointsPerCup:20,pointsPerTrny:50,prizeTrack:[{pts:100,reward:100},{pts:250,reward:250},{pts:500,reward:550},{pts:1000,reward:1200}]},
    economyPlus:{bondRate:2,bondMin:100,mineEnergyCost:10,mineSuccessPct:60,mineMaxYield:40,exchangeFeePct:10,exchangeDailyCap:500},
...
function renderFeatureAdmin(){
  ...
  const economy=(ep.cratesOpened||0)+(ep.tradingListings?.length||0)+(stake.balance||0)+(utility.crafts?.length||0)+(utility.ticketPurchases?.length||0)+(utility.clanTreasury||0)+(utility.roomPurchases?.length||0)+(ep.auction?.history?.length||0)+(ep.bonds?.principal||0)+(ep.mine?.digs||0)+(ep.exchange?.log?.length||0);
...
const ADMIN_CATALOG_GAMES=[['overunder','Over / Under'],['speed','Speed Round'],['tug','Tug of War'],['evenodd','Even / Odd Sum'],['closest','Closest Number'],['luckybattle','Lucky Number Battle'],['sumpredict','Sum Prediction'],['higherbyte','Higher Byte'],['patternrace','Pattern Race'],['parlayduel','Parlay Duel'],['prediction','Prediction Streak'],['blind','Blind Pick'],['rangewar','Range War'],['bullseye','Bullseye'],['chain','Chain Reaction'],['ladder','Elimination Ladder'],['mirrored','Mirrored Coins'],['rps','Rock Paper Scissors Duel'],['closest21','Closest to 21'],['triplecoin','Triple Coin Majority'],['sequencebuilder','Sequence Builder'],['dicesumduel','Dice Sum Duel'],['colourspectrum','Colour Spectrum Duel'],['primecomposite','Prime vs Composite'],['medianbattle','Median Number Battle'],['streaksurvivor','Streak Survivor'],['territory','Territory Capture'],['modulo4','Modulo Four Duel'],['pokerhigh','Poker High Duel'],['threedicepoker','Three Dice Poker'],['lastdigit','Last Digit Duel'],['binaryduel','Binary Code Duel'],['coinbalance','Coin Balance Battle'],['bytewar','Byte Showdown'],['sumfour','Sum of Four'],['highcard','High Card Duel'],...P2P_EXTRA.map(g=>[g.id,g.name])];
const ADMIN_ARCADE_GAMES=[['wheel','Lucky Wheel'],['scratch','Scratch Cards'],['dice','Dice Roll'],['raffle','Weekly Raffle'],['ladder','Multiplier Ladder P2P'],['war','War Card Game'],['crash','Crash'],['hilo','Hi-Lo'],['mines','Mines'],['plinko','Plinko Drop'],['slots','Mini Slots'],['keno','Quick Keno'],['bingo','Bingo Rush'],['treasure','Treasure Hunt'],['memory','Memory Match'],['dropball','Drop Ball'],['trivia','Daily Trivia'],['fishing','Fishing Reel'],['penalty','Penalty Shootout'],['coinpusher','Coin Pusher'],['tower','Tower Builder'],['match3','Match-3 Rush'],['vault','Mystery Vault'],['roulette','Mini Roulette'],['blackjack','Blackjack Hit-or-Stand'],...Object.values(EXT_ARCADE2).map(g=>[g.id,g.title])];
...
const ADMIN_ROLES={
 'Super Admin':['dash','ops','people','features','directory','rates','econ','revenue','topups','withdraw','promo','vip','plus','trny','referrals','announcements','support','reports','games','approvals','audit','trust','compliance','settings'],
 'Finance':['dash','econ','revenue','topups','withdraw','promo','audit','reports','compliance'],
 'Operations':['dash','ops','people','features','directory','vip','plus','trny','approvals','promo','games','announcements','support'],
